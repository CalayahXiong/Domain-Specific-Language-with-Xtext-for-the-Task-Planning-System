...
public class AutoTPS {
    ...
    public void assignTasks() {
        ...
        // Clear previous assignments if necessary
        assignments.clear();
        ...
        for (Task task : tasks) {
            ...
            for (Rule rule : rules) {
                if (ruleMatchesTask(rule, task)) {
                    ...
                    // If no active worker with required seniority exists, output a failure message.
                    if (candidates.isEmpty()) {
                        ...
                        taskAssigned = true;  // We consider the task "processed"
                        break; // exit rule loop for this task
                    }
                    ...
                    for (Worker candidate : candidates) {
                        if (isWorkerAvailable(candidate, task)) {
                            ...
                            // Candidate is not available; find the overlapping task.
                    ...
                    if (taskAssigned) {
                        // If the task was assigned successfully, no need to check further rules.
                        break;
                    }
                }
            }
